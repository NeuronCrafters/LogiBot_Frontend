...
function useSubjectsData(filter: CategoryChartProps['filter']) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (isStudent) {
        return await logApi.get(
            'student',
            'subjects',
            'individual',
            universityId!,
            { courseId, classId, studentId: targetId }
        );
      }
      switch (filter.type) {
        case 'university':
...
export function CategoryChart({ filter }: CategoryChartProps) {
  ...
  const hasData = useMemo(() => data?.totalAccesses > 0, [data]);
  const topCategory = useMemo(() => {
    ...
    const labels: Record<string,string> = {
      variaveis: 'Variáveis', tipos: 'Tipos', funcoes: 'Funções', loops: 'Loops', verificacoes: 'Verificações'
    };
        ...
        <CardContent className="px-2 sm:p-6 h-[299px] flex items-center justify-center">
          {(!hasRequired || isLoading || isError || !hasData) && (
              <div className="flex flex-col items-center">
                ...
                {!isLoading && !isError && hasRequired && !hasData && <p className="text-white/70">Nenhum dado de categorias disponível para esta entidade.</p>}
